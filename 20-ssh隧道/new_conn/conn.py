...
class SSHConnection(object):
    ...
    def do_command(self, command, timeout=5):
        """
        执行命令
        :param command: 命令
        :param timeout: 超时
    ...
    def recv(self):
        ...
        self.chan_is_open()
        try:
            x = self.chan.recv(65535).decode("utf-8")
            if len(x) == 0:
                return
            return x
        except:
    ...
    def slave_do_command(self, command, timeout=5):
        """
        执行命令
    ...
    def slave_recv(self):
        ...
        self.slave_chan_is_open()
        try:
            x = self.slave_chan.recv(65535).decode("utf-8")
            if len(x) == 0:
                return
            return x
        except:
